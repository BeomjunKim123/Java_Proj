...
interface A {
	...
	default void cde() {
		System.out.println("2010년도에 USB 3.0 기능 구현");
	}; //2010년도에 구현된 메소드  <== 새로운 기능을 1000개의 클래스에 적용 할려고 한다.
...
class Aa implements A {
	...
	@Override
	public void cde() {
		System.out.println("부모의 default 메소드 오버라이딩 됨.");
	}
	...
	public static void main(String[] args) {
		/*
		 	인터페이스의 특징 : 
		 		...
		 		- 인터페이스 내부의 메소드는 선언만 되어야하고 {}를 생성하면 안된다. <== 자바 1.7
		 			- default / static <== 구현부를 가지는 메소드 <== 자바 1.8 추가된 기능
		 	default : 메소드 앞에 붙이는 키 <== 인터페이스에서 구현부가 있는 메소드, 주의 : 접근 제어자가 아니다.
		 		- 인터페이스에서 구현된 모든 클래스에게 상속되어서 적용.
		 		- 새로운 기능을 하위의 모든 클래스에게 적용 할때 사용함. 오류없이 적용할때 사용
		 		- 오버라이딩도 가능함.
