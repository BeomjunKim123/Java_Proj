...
class Bb implements B {
	...
	@Override
	public void abc() {
		System.out.println("B 인터페이스를 구현한 메소드 abc() ");
		...
		/*
		 	인터페이스 (interface) : 내부와 외부를 연결 시켜주는 접점. 콘센트(가전제품 <==> 전기), 메뉴판 (손님 <==> 주방장)
		 		...
		 		- 모든 메소드는 public abstract 가 생략 되어 있다. <== 선언만 된 메소드
		 			단 예외가 있다. (default, static 메소드는 제외 <== 자바 1.8부터 적용됨)
		 		- 인터페이스는 객체화 할 수 없다. 단 타입으로 지정은 가능하다.
		 		- 인터페이스를 구현한 클래스는 여러 인터페이스를 구현 가능 하다.
		 		- 클래스는 단 하나의 부모만 상속 할 수 있다. (다중 상속이 불가능하다) extends
		 		- 클래스는 여러 인터페이스를 구현할 수 있다. (다중 상속이 가능하다) implements
		 		
		 		- 유지 보수를 쉽게 하기 위해서 Interface를 사용함. 강결합 ==> 약결합 (유지 보수 쉽게)
