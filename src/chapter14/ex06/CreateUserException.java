...
class A {
	...
	void abc_2(int num) throws MyException {  //abc_2() 메소드를 호출 하는 곳에서 예외를 처리해야함.
		if ( num > 70 ) {
			...
			//예외 강제 발생 시킴 (trows : 예외를 미루기, throw : 예외를 강제로 발생 시킴)
			throw mre1;
		}
	}
	void bcd_2() {
...
public class CreateUserException {
	...
	public static void main(String[] args) {
		/*
		 	사용자 정의 예외 : Java에서 제공되지않는 예외를 자신이 직접 만들어서 사용하는 예외
		 	 	- 일반예외(checked Exception) : 컴파일 단계에서 오류가 발생, 예외 처리가 반드시 필이ㅛ
		 	 		- Exception 클래스를 상속해서 사용자 정의 예외 (1.기본생성자, 2.오류메세지를 처리할 생성자)
		 	 	- 실행예외(uncheked Exception, RuntimeException) : 실행시 예외 발생.
		 	 		- RuntimeException 클래스를 상속해서 사용자 정의 예외 (1.기본생성자, 2.오류 메세지를 처리할 생섲자)
		 */
