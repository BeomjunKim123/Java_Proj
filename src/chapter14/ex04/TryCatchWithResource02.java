...
public class TryCatchWithResource02 {
	...
	public static void main(String[] args) {
		/*
		 	...
		 	-일반적인 try 구문 : try { 실행블락 (예외) } catch (예외 e ) {예외를 처리할 블락}
		 				finally {객체제거}
		 				
		 	//객체가 자동으로 close 됨
		 	- try (리소스 선언 부분) {실행블락} catch (예외 e) {예외를 처리할 블락}
		 	
		 	//자동으로 객체를 제거 할려면 1.AutoClosable() 인터페이스를 구현한 클래스만이 자동으로 close() 할 수 있다.
		 	 * 1. AutoClosable() 인터페이스를 구현한 클래스만이 자동으로 close() 할 수 있다.
		 	 * 2. try (객체선언) {} catch (예외 e) {]
		 */
		...
		//finally 블락에서 객체를 제거하지 않아도 자동으로 객체가 제거됨.
	}
