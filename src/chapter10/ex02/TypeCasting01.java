...
public class TypeCasting01 {
	public static void main(String[] args) {
		/*
		  객체의 타입 변환 (object Type Casting) :
		  	-업캐스팅(upcasting) : 자식 객체 ==> 부모 타입으로 변환, 명시하지 않아도 자동 변환됨.
		  		객체를 배열/컬렉션에 저장하기 위해서 동일한 타입으로 배열이나 컬렉션에 저장
		  		
		  	-다운캐스팅(downcasting) : 부모타입 ==> 자식 타입으로 변환, 명시해서 변환
		...
		//a1는 Animal 타입만 존재 : Animal 클래스의 필드나 메소드만 접근
		...
		Animal a2 = t1; //Tiger ==> Animal 업캐스팅 (Animal 타입의 필드나 메소드만 접근이 가능)
	
		//a2는 Animal 타입으로 업캐스팅 : Animal의 필드/메소드만 접근 가능
		...
		//다운 캐스팅 : 부모 ===> 자식 변환, 반드시 instanceof를 사용해서 실행시 오류나지 않도록 처리가 필요함.
		Tiger tt1 = (Tiger) a2;
		...
		//3. Lion 객체를 생성 : l1 : lino/Animal
			//자식 클래스를 객체화 시키면 부모 클래스의 필드/메소드
		Lion l1 = new Lion();
		...
		//다운캐스팅 : Animal(부모) => Lion(자식)
			...
			//a1 : Animal
			 //객체 내부에 해당 타입이 존재하지 않는 경우 : 
			//실행시 오류가 발생 /  컴파일 단계에서는 오류가 발생되지 않음.
			...
			if(a5 instanceof Lion) {
				Lion ll71 = (Lion) a5;
				System.out.println("a5는 Lion 타입으로 잘 캐스팅 되었습니ㅏㄷ.");
			} else {
