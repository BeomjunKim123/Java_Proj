...
//부모 클래스 (Human) <== 필드와 메소드 : 자식클래스에서 공통된 특징을 갖는 필드와 메소드를 정의 함.
class Human { //부모클래스
	
	String name; //모든 사람은 이름 갖고 있다.
	int age; //모든 사람은 나이르 갖고 있다.
...
//자식 클래스 (Student(학생), Worker(근로자), Professor(교수))
	...
	//추가적으로 그 클래스만이 갖고있는 필드와 메소드를 정의해서 사용.
...
public class Inheritance01 {
	public static void main(String[] args) {
		/* 상속 (Inheritance) : 부모 클래스의 필드 (객체화) 의 필드(속성)와 메소드(기능),
		 					이너클래스를 자식클래스에서 상속받아서 사용.
			- 생성자는 상속되지 않는다.
			- 중복된 코드를 방지 할 수 있다.
			- 객체 지향언어의 꽃이다.
			- 다형성 : 부모 클래스의 내용을 상속 받아서 다양한 형태로 출력 : 동물 (부모클래스) <== 사람, 독수리, 사자, 호랑이, 개, 고양이
					-- 부모클래스의 필드와 메소드는 자식클래스로 내려감.
