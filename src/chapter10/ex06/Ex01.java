...
public class Ex01 {
	...
	//각 객체의 필드의 값 할당은 생서자 호출시 매개변수로 값을 넣도록 하세요. 
		...
		// 배열로 변수 선언
		int[] sum = new int[3]; //sum[0] : 국어점수의 합, sum[1] : 영어점수의 합, sum[2] : 수학점수의 합
		
		//Student[] arr = new Student[] {s1, s2, s3, s4, s5} ;
		
		for (int i = 0; i < arr.length; i++) {
			
			sum[0] += ((Student_Sub)arr[i]).kor; //sum[0] = sum[0] + kor   //다운캐스팅
			sum[1] += ((Student_Sub)arr[i]).eng; //sum[1] = sum[1] + eng
			sum[2] += ((Student_Sub)arr[i]).math; //sum[2] = sum[2] + math
			
		}
		System.out.println("모든 사용자의 국어점수의 총합은 : " + sum[0]);
		System.out.println("모든 사용자의 영어점수의 총합은 : " + sum[1]);
		System.out.println("모든 사용자의 수학점수의 총합은 : " + sum[2]);
