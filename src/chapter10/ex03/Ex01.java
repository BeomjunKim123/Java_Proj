...
// 부모 클래스 : 자식클래스의 공통적으로 적용되는 필드, 메소드 정의
...
class Worker extends Human{
	...
	@Override
	public String toString() {
		return "Worker [workID=" + workID + ", name=" + name + ", age=" + age + "]";
	}
...
public class Ex01 {
	public static void main(String[] args) {
		...
		Human h1 =  new Human();
			...
			//h1 은 Human을 객체화 했기 때문에 Student / Professor / Worker 클래스를 내포하고 있지 않아서 다운 캐스팅 할수 없다.
		...
		//오류 : 컴파일 오류 (이클립스에서 오류를 발생 시킴), 런타임오류(실행시 오류 발생)
		...
		Human s2 = new Student(); //s2 : Human / Student 내포하고있고, Human 타입으로 지정되어 있다.
		...
		//ss2 : Human / student 타입을 내포하고 있고 Student 타입으로 다운캐스팅 : Human / student 필드 메소드를 모두 사용 가능
		ss2.name = "세종대왕"; //Human
		ss2.age = 50; //Human
		...
		Human hh1 = ss2; //업캐스팅은 캐스팅시 instnaceof를 사용하지 않아도 됨.. 컴파일 단계에서 오류가 발생됨.
		...
		Student s20 = (Student) arr[0]; //Human ==> Student 타입으로 다운캐스팅
		...
		//3-2. Student 객체 생성후 Student 타입으로 지정
		...
		h40.eat(); //Human 클래스의 eat() 메소드를 호출시, Student 클래스의 오버라이딩된 eat()이 작동됨
		h41.eat(); //Human 클래스의 eat() 메소드를 호출시, Professor 클래스의 오버라이딩된 eat()이 작동됨
		h42.eat(); //Human 클래스의 eat() 메소드를 호출시, Worker 클래스의 오버라이딩된 eat()이 작동됨
