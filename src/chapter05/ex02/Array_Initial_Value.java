...
public class Array_Initial_Value {
	public static void main(String[] args) {
		/*
		  배열의 초기값 지정 : 
		  	참조 자료형 : boolean : false, 정수(byte, short, int, long) : 0
		...
		int val1; //변수선언
		//System.out.println(val1); //오류 발생, 변수의 기본 값을 할당 하지 않아서 오류 발생
		
		int[] val2; // 배열 선언, Heap 영역의 주소 값이 없는 상태
		//System.out.println(val2); //오류 발생, Heap영역이 생성되지 않는 상태, new int[3]
		
		val2 = new int[3]; //heap영역에 new라는걸 생성해라
		...
		int[] val3 = null; //참조 자로형일때만 객체 주소에 null 값 할당
		...
		int [] b = new int[3]; //정수 : byte, int, long, short
