...
//MyClass2는 TreeSet에 저장할때 특정 필드가 정렬해서 저장되도록 함.
...
public class Set_TreeSet_Class {
	...
	public static void main(String[] args) {
		/*
		 	TreeSet <E> : 1. Set<E> 을 인터페이스를 구현한 클래스, 2. 중복된 값을 넣을수 없다. 3. 방번호가 없다.
		 		4. 값이 정렬되어서 들어간다(오름차순 정렬), 5. 검색/정렬 기능의 메소드가 추가적으로 들어있다.
		 		- Wapper Class 는 자동으로 정려해서 들어 가도록 설계가 되어있다. String
		 		
		 		- 일반 클래스를 TreeSet에 저장할때 특정 컬럼을 정렬해서 저장 되도록 하기 위해서는
		 		- Comparable<E> 인터페이스를 구현해서 comparTo() 메소드를 오버라이딩해서 특정컬럼이 정렬되어 들어가도록 설계해야한다.
		 */
		...
		set2.add(50); set2.add(50); set2.add(30); set2.add(50);
		System.out.println(set2);
		
		//특정 객체를 TreeSet에 넣으면 정렬되어 저장 되지 않습니다.
		//	- Comparable<E> 인터페이스를 구현해서 compareTo() 메소드를 오버라이딩해서 특정칼럼이 정렬
		//	- 중복되지 않도록 할려면 : Object 클래스의 equals
		
		
		System.out.println("=========================");
		//1. MyClass 객첼를 TreeSet에 저장 <== 일반 클래스 : 정렬되어 들어가지 않는다.
		...
		//2. Myclass2 객체를 TreeSet에 저장 <== Comparable 인터페이스의 comparTo() 메소드를 재정의 함 : data1 : data1 필드를 정렬
